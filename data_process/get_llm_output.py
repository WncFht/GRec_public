...
def get_user_preference(args, inters, item2feature, reviews, api_info):
    ...
    with open(preference_output_file, mode="a") as f:
        while st < len(prompt_list_1):
            ...
            for i, answers in enumerate(zip(res_1, res_2, strict=False)):
                user = users[st + i]
...
if __name__ == "__main__":
    ...
    with open(intention_train_output_file) as f:
        for line in f:
            # print(line)
    ...
    with open(intention_test_output_file) as f:
        for line in f:
            content = json.loads(line)
            if content["user"] not in intention_train:
    ...
    with open(preference_output_file) as f:
        for line in f:
            content = json.loads(line)
