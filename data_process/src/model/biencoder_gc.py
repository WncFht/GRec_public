...
class BiEncoderGradCache(nn.Module):
    ...
    def split_inputs(self, model_input, chunk_size: int) -> list:
        ...
        if isinstance(model_input, list) and all(
            ...
            return [list(s) for s in zip(*chunked_x, strict=False)]
        ...
        if isinstance(model_input, tuple) and list(map(type, model_input)) == [
            ...
            return list(zip(args_chunks, kwargs_chunks, strict=False))
    ...
    def forward_no_grad(
        ...
        with torch.no_grad():
            for x in zip(*model_inputs, strict=False):
    ...
    def cache_step(
        ...
        **loss_kwargs,
        ...
        inputs = [
            ...
            for x, chunk_size in zip(inputs, self.chunk_sizes, strict=False)
        ]
        masks = [
            self.split_inputs(x, chunk_size)
            for x, chunk_size in zip(masks, self.chunk_sizes, strict=False)
        ]

        for model, input, mask in zip(self.models, inputs, masks, strict=False):
        ...
        cache = [
            c.split(chunk_size)
            for c, chunk_size in zip(cache, self.chunk_sizes, strict=False)
        ]

        for model, input, mask, model_cache, rnd_states in zip(
            self.models, inputs, masks, cache, all_rnd_states, strict=False
        ):
            self.forward_backward(
                model,
                model_inputs=list(zip(input, mask, strict=False)),
