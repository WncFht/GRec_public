    ...
    def ui_guide_forward(
        ...
        **kwargs,
        ...
        if patch_pos[0] is not None:
            ...
            assert select_mask[0] is not None, (
            ...
            for idx, (patches_count, comps_count) in enumerate(grid_dims):
                ...
                if disabled.numel() > 0:
                    ...
                    ).unsqueeze(1)  # shape [merge, 1]
                    dx = torch.arange(
                        self.spatial_merge_size, device=hidden_states.device
                    ).unsqueeze(0)  # shape [1, merge]
                    # Offsets in the premerged flattened grid for one merged block.
                    offsets = (
                        dy * grid_w + dx
                    ).flatten()  # shape [merge*merge]
...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        position_embeddings: tuple[torch.Tensor, torch.Tensor]
        | None = None,  # necessary, but kept here for BC
        patch_pos: torch.LongTensor | None = None,
        select_mask: torch.LongTensor | None = None,
        **kwargs,
    ) -> tuple[
    ...
    def ui_guide_forward(
        ...
        **kwargs,
        ...
        if patch_pos[0] is not None:
            ...
            if use_cache:
                processed_hidden_states[:, retain_mask] = block_outputs[
                    0
                ].flatten(0, 1)
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [
                Qwen2VLVisionBlock(
                    config, config._attn_implementation, layer_idx=i
                )
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                image_grid_thw = [
                    image_grid_thw[i]
                    if image_grid_thw[i] is not None
                    and image_grid_thw[i].sum() > 0
                    else None
                    for i in range(bsz)
                ]
                idx_w_image = [
                    mid
                    for mid, isize in enumerate(image_grid_thw)
                ...
                if len(idx_w_image) > 0:
                    ...
                    if idx_w_image:
                        ...
                        inputs_embeds_w_image = (
                            inputs_embeds_w_image.masked_scatter(
            ...
            if pixel_values_videos is not None:
                video_grid_thw = [
                    video_grid_thw[i]
                    if video_grid_thw[i] is not None
                    and video_grid_thw[i].sum() > 0
                    else None
                    for i in range(bsz)
                ]
                idx_w_video = [
                    mid
                    for mid, isize in enumerate(video_grid_thw)
                    if isize is not None
                ]
                idx_wo_video = [
                    mid
                ...
                if len(idx_w_video) > 0:
                    ...
                    if idx_w_video:
                        ...
                        inputs_embeds_w_video = (
                            inputs_embeds_w_video.masked_scatter(
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position[0] == 0:
            patch_pos = kwargs.get("patch_pos")
            select_mask = kwargs.get("select_mask")
        else:
            patch_pos = None
            select_mask = None
