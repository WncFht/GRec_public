...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    #     # 处理不同维度的 grid_thw
    #     if grid_thw.dim() == 1:
    #         # 如果是1维张量 [t, h, w]，转换为2维以统一处理
    #         grid_thw = grid_thw.unsqueeze(0)  # [t, h, w] -> [[t, h, w]]
    #     elif grid_thw.dim() != 2:
    #         raise ValueError(f"Unexpected grid_thw shape: {grid_thw.shape}")

    #     # 现在 grid_thw 肯定是2维的，可以统一处理
    #     for i in range(grid_thw.shape[0]):
    #         t, h, w = grid_thw[i]

    #         # 确保 t, h, w 是标量值
    #         t = t.item() if hasattr(t, 'item') else int(t)
    #         h = h.item() if hasattr(h, 'item') else int(h)
    #         w = w.item() if hasattr(w, 'item') else int(w)
    ...
    #     # 确保 max_grid_size 是标量
    #     max_grid_size = grid_thw[:, 1:].max()
    #     if hasattr(max_grid_size, 'item'):
    #         max_grid_size = max_grid_size.item()
