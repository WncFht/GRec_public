...
class Phi3SuScaledRotaryEmbedding(Phi3RotaryEmbedding):
    def __init__(self, dim, config, device=None):
        super().__init__(
            dim, config.max_position_embeddings, config.rope_theta, device
        )
...
class Phi3Attention(nn.Module):
    ...
    def __init__(self, config: Phi3VConfig, layer_idx: int | None = None):
        super().__init__()
        self.config = config
...
class Phi3VModel(Phi3VPreTrainedModel):
    ...
    def forward(
        ...
        idx_wo_image = [
            mid for mid, isize in enumerate(image_sizes) if isize is None
        ]
        inputs_embeds = []
        if idx_w_image:
            valid_pixel_values = [
