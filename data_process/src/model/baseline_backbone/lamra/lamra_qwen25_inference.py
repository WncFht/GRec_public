...
from src.model.vlm_backbone.qwen2_5_vl import (
...
class LamRAQwen25VL(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"model.device: {self.base.device!s}")
        ...
        if inputs_embeds is None:
            ...
            if has_image:
                if type(pixel_values) is list:
                    ...
                    ).to(input_ids.device)  # shape=[BS*n_patch,C*H*W]
                    image_grid_thw = torch.cat(
                        [torch.from_numpy(thw) for thw in image_grid_thw]
                    ).to(input_ids.device)  # shape=[BS,H,W]
    ...
    def embed(
        ...
        **kwargs,
        ...
        for t, img_path in zip(texts, images, strict=False):
