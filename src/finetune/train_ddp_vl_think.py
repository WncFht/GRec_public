...
class AnswerOnlyTrainer(Trainer):
    ...
    @torch.no_grad()
    def _generate_reasoning_chain(self, model, inputs, max_new_tokens=512):
        """
        让模型生成 <think>...</think><answer>...</answer>
        返回生成的 input_ids（含 prompt）
        """
        gen_kwargs = {
            ...
            "use_cache": False,  # 关掉以免与 gc 冲突
        }
        # 把 pixel_values / image_grid_thw 等视觉键值带过去
        for k in [
            "pixel_values",
            "image_grid_thw",
            "pixel_values_videos",
        ...
        output_texts = self.processing_class.tokenizer.batch_decode(
            output_ids, skip_special_tokens=False
        )
        for t in output_texts:
            print(t.replace("<|endoftext|>", ""))
            print("-" * 50)

        return gen_out["sequences"]  # (B, L_total)
    ...
    def compute_loss(
        ...
        # 1. 先让模型生成完整轨迹（无梯度）
        gen_ids = self._generate_reasoning_chain(model, inputs)

        # 2. 构造 labels：只有 <answer> 区间保留，其余 -100
        ...
        for i, seq in enumerate(gen_ids):
            ...
            if m:
                ans_start_char, ans_end_char = m.start(1), m.end(1)
                enc = tokenizer(
                ...
                try:
                    ...
                    # 把 answer 区间写回 labels
                    labels[i, ans_tok_start:ans_tok_end] = gen_ids[
                        i, ans_tok_start:ans_tok_end
                    ]
                except StopIteration:
                    pass  # 没找到就保持全 -100

        # 3. 再 forward 一次，这次有梯度，只算 answer 的 loss
        model_inputs = {
            "input_ids": gen_ids,
            "attention_mask": (gen_ids != tokenizer.pad_token_id).long(),
        }
