...
class UnifiedTrainer:
    ...
    def _log_statistics(self, train_data, original_vocab_size, new_vocab_size):
        ...
        if self.local_rank == 0:
            if self.args.use_lora:
                self.logger.info(f"LoRA config: r={self.args.lora_r}, alpha={self.args.lora_alpha}, dropout={self.args.lora_dropout}")
                self.logger.info(f"Target modules: {self.args.lora_target_modules}")
                if hasattr(self.args, 'lora_modules_to_save') and self.args.lora_modules_to_save:
                    self.logger.info(f"Modules to save: {self.args.lora_modules_to_save}")
            ...
            self.logger.info(f"Steps per epoch: {len(train_data) / self.args.per_device_batch_size:.2f}")
    ...
    def _save_configs(self, processor, new_vocab_size):
        ...
        config = AutoConfig.from_pretrained(self.args.base_model, trust_remote_code=True)
    ...
    def _save_new_token_embeddings(
        ...
        embedding_save_path = os.path.join(self.args.output_dir, "new_token_embeddings.pkl")
