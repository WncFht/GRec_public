...
class SeqRecDataset(BaseDataset):
    def __init__(
        ...
        # load data
        self._load_data()  # load self.inter
        self._remap_items()  # 交互序列转化为token形式
...
class MultimodalDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(input_ids=input_data, labels=output_data)
...
class TextEnrichDataset(BaseDataset):
    ...
    def __init__(self, args, prompt_sample_num=1, sample_num=-1):
        ...
        self.prompts = self.prompt["text_enrich"]
