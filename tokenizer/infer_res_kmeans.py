...
def main():
    ...
    parser.add_argument(
        "--emb_path", type=str, required=True, help="embedding file path"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=None,
        help="output path (default: emb_path + _codes.parquet)",
    )
    parser.add_argument(
    ...
    model.eval()
    print(
        f"Model loaded: n_layers={model.n_layers}, codebook_size={model.codebook_size}, dim={model.dim}"
    )
