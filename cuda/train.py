...
def stress_gpu(device_id, duration=None, memory_ratio=0.95):
    ...
    if allocated_bytes > 0:
        print(
            f"[GPU {device_id}] 已分配 {(allocated_bytes / 1024 / 1024):.1f} MB "
        ...
        try:
            while True:
                ...
                if iterations % 1000 == 0:
                    ...
                    print(
                        f"[GPU {device_id}] 运行 {elapsed:.1f} 秒, "
                        f"迭代次数: {iterations}"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--list-gpus", action="store_true", help="列出可用GPU")
    ...
    if args.list_gpus:
        ...
        if available_gpus:
            for i in available_gpus:
                ...
                print(
                    f"  GPU {i}: {props.name} ({props.total_memory / 1024 / 1024 / 1024:.1f} GB)"
                )
        sys.exit(0)
